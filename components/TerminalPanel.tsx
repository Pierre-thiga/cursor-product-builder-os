      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 bg-white scroll-smooth">
        ...
        <AnimatePresence mode='popLayout'>
          {flow.steps.map((step, idx) => {
            if (idx > currentStepIndex + 1) return null; // Show current + history, not future
              ...
              >
                ...
                <div className="flex-1 bg-slate-50/50 rounded p-2 border border-slate-100/50">
                  ...
                  <div className="flex items-center gap-1.5 mb-2 text-[10px] font-mono text-slate-400">
                    <span className="bg-white border border-slate-200 px-1 rounded truncate max-w-[80px]" title={step.from}>{getNodeLabel(step.from)}</span>
                    <span>→</span>
                    <span className="bg-white border border-slate-200 px-1 rounded truncate max-w-[80px]" title={step.to}>{getNodeLabel(step.to)}</span>
                  </div>
